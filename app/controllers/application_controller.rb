...
class ApplicationController < ActionController::API
  ...
  rescue_from(ActiveRecord::RecordNotFound) do |error|
    handle_active_record_not_found_error(error)
  end
  ...
  rescue_from(ActiveRecord::RecordNotUnique) do |error|
    handle_active_record_not_unique_error(error)
  end
  rescue_from(ActionController::ParameterMissing) do |error|
    handle_action_controller_parameter_missing_error(error)
  end
  rescue_from(ActionController::RoutingError) do |error|
    handle_action_controller_routing_error(error)
  end
